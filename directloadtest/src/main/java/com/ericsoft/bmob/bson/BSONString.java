...
public class BSONString {
    ...
    public LinkedHashMap<String, Object> getBSON(String str) {
        ...
        List<String> bsons = this.checkArraySize(str);
        if (bsons.size() == 1) {
            String[] strs = this.formatOneBSON(bsons.get(0));
            if (strs == null) {
                return map;
            }
            ...
            for (int i = 0; i < bsons.size(); i++) {
                String[] strs = this.formatOneBSON(bsons.get(i));
                if (strs == null) {
                    return map;
                }
                map.put(strs[0], this.getValue(strs[1]).get(BSON.TAG_VALUE));
            }
    ...
    public LinkedHashMap<String, Object> getValue(String str) {
        ...
        if (str.startsWith(BSON.CHAR_LEFT_MID) && str.endsWith(BSON.CHAR_RIGHT_MID)) {
            ...
            List<String> array = this.checkArraySize(str.substring(1, str.length() - 1));
            Object[] objArray = new Object[array.size()];
            for (int i = 0; i < array.size(); i++) {
                LinkedHashMap<String, Object> tMap = this.getValue(array.get(i));
            ...
            map = this.analyseObject(str, map);
            type = (Integer) map.get(BSON.TAG_TYPE);
            map.put(BSON.TAG_VALUE, map.get(BSON.TAG_VALUE));
        } else if ("true".equals(str.toLowerCase(Locale.getDefault())) || "false".equals(str.toLowerCase(Locale.getDefault()))) {
    ...
    private LinkedHashMap<String, Object> analyseObject(String str, LinkedHashMap<String, Object> map) {
        LinkedHashMap<String, Object> oMap = this.getBSON(str);
        ...
        if (msg.equals(BSON.MSG_BSON_ARRAY)) {
            ...
            Iterator<Map.Entry<String, Object>> iter = oMap.entrySet().iterator();
            while (iter.hasNext()) {
        ...
        if (!tag.equals(BSON.MSG_ERROR_EMPTY) && !tag.equals(BSON.MSG_ERROR_NOTBSON)) {
            map.put(BSON.TAG_TYPE, BSON.TYPE_BSON);
            BSONObject bson = new BSONObject();
            bson.put(tag, this.getValue((String) oMap.get(BSON.TAG_VALUE)).get(BSON.TAG_VALUE));
            map.put(BSON.TAG_VALUE, bson);
        } else {
